...
public class LoginEndpoint(
    ...
    IHubContext<NotificationHub> notificationHubContext) : Endpoint<LoginRequest, ResponseData<LoginResponse>>
{
    ...
    public override async Task HandleAsync(LoginRequest req, CancellationToken ct)
    {
        ...
        await using var sessionLock = await distributedLock.AcquireAsync(
            $"auth:session-lock:{loginInfo.UserId.Id}",
            cancellationToken: ct);

        var nowTime = DateTimeOffset.UtcNow;
        var tokenExpiryTime = nowTime.AddMinutes(appConfiguration.Value.TokenExpiryInMinutes);
        var refreshToken = refreshTokenGenerator.Generate();
        var sessionId = TokenGenerator.GenerateCryptographicallySecureGuid();
        ...
        var claims = BuildClaims(loginInfo, dataScope, deptId, authorizedDeptIds, sessionId);
        ...
        await userSessionService.ReplaceAsync(
            loginInfo.UserId.Id,
            sessionId,
            tokenExpiryTime - nowTime);

        await notificationHubContext.Clients
            .User(loginInfo.UserId.Id.ToString())
            .SendAsync("SessionReplaced", new { loginTime = nowTime }, ct);
    ...
    private static List<Claim> BuildClaims(
        UserLoginInfoQueryDto loginInfo,
        DataScope dataScope,
        DeptId? deptId,
        ...
        string sessionId)
    {
        ...
        [
            ...
            new Claim(UserSessionClaimTypes.SessionId, sessionId),
            new Claim("data_scope", ((int)dataScope).ToString()),
            new Claim("dept_id", deptId?.Id.ToString() ?? string.Empty),
            new Claim("authorized_dept_ids", string.Join(",", authorizedDeptIds.Select(d => d.Id)))
        ];
    }
}
