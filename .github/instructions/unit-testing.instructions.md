...
- 领域层测试：`test/Ncp.Admin.Domain.Tests/{EntityName}Tests.cs`。
- Web 层测试：`test/Ncp.Admin.Web.Tests/{Feature}Tests.cs`。
- 基础设施层测试：`test/Ncp.Admin.Infrastructure.Tests/{Component}Tests.cs`。
