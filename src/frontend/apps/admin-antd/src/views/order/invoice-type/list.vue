...
} from '#/api/system/order-invoice-type';
...
const [Grid, gridApi] =
  useVbenVxeGrid<OrderInvoiceTypeOptionApi.OrderInvoiceTypeOptionItem>({
    formOptions: {
      ...
      proxyConfig: {
        ajax: {
          query: async (
            ...
            const list = await getOrderInvoiceTypeOptionList();
            let items = list;
            if (formValues?.name) {
...
function onActionClick(
  e: OnActionClickParams<OrderInvoiceTypeOptionApi.OrderInvoiceTypeOptionItem>,
) {
  if (e.code === 'edit') {
    ...
    Modal.confirm({
      ...
      onOk: async () => {
        await deleteOrderInvoiceTypeOption(e.row.id);
...
<template>
  <Page auto-content-height>
    ...
    <Grid :table-title="$t('order.invoiceTypeList')">
      <template #toolbar-tools>
        ...
        >
          ...
          {{ $t('ui.actionTitle.create', [$t('order.invoiceTypeList')]) }}
        </Button>
      </template>
    </Grid>
  </Page>
