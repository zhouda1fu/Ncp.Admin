...
Ncp.Admin.sln
├── src/
│   ├── Ncp.Admin.Domain/         # 领域层 - 聚合根、实体、领域事件
│   ├── Ncp.Admin.Infrastructure/ # 基础设施层 - EF配置、仓储接口、仓储实现
│   └── Ncp.Admin.Web/           # 表现层 - API、应用服务
└── test/                            # 测试项目
    ├── Ncp.Admin.Domain.UnitTests/         # 领域层测试项目
    ├── Ncp.Admin.Infrastructure.UnitTests/ # 基础设施层测试项目
    └── Ncp.Admin.Web.UnitTests/           # 表现层测试项目
```
...
### 文件组织
- **聚合根** → `src/Ncp.Admin.Domain/AggregatesModel/{AggregateFolder}/`
...
- **命令与命令处理器** → `src/Ncp.Admin.Web/Application/Commands/`
- **查询与查询处理器** → `src/Ncp.Admin.Web/Application/Queries/`
- **API端点** → `src/Ncp.Admin.Web/Endpoints/`
- **领域事件处理器** → `src/Ncp.Admin.Web/Application/DomainEventHandlers/`
- **集成事件** → `src/Ncp.Admin.Web/Application/IntegrationEvents/`
- **集成事件转换器** → `src/Ncp.Admin.Web/Application/IntegrationEventConverters/`
- **集成事件处理器** → `src/Ncp.Admin.Web/Application/IntegrationEventHandlers/`
...
**Web层** (`src/Ncp.Admin.Web/GlobalUsings.cs`):
...
**Infrastructure层** (`src/Ncp.Admin.Infrastructure/GlobalUsings.cs`):
...
**Domain层** (`src/Ncp.Admin.Domain/GlobalUsings.cs`):
...
**查询处理器**:
```csharp
using Ncp.Admin.Domain.AggregatesModel.{AggregateFolder};
...
```csharp
using Ncp.Admin.Domain.AggregatesModel.{AggregateFolder};
using Ncp.Admin.Web.Application.Commands.{FeatureFolder};
using Ncp.Admin.Web.Application.Queries.{FeatureFolder};
```
