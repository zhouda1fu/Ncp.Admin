...
using OrderAggregateRoot = Ncp.Admin.Domain.AggregatesModel.OrderAggregate.Order;

namespace Ncp.Admin.Web.Application.Commands.Orders;

/// <summary>
/// 订单工作流变量（存入 Variables JSON，供条件分支评估）
/// </summary>
public class OrderWorkflowVariables
{
    ...
    /// <summary>
    /// 按产品分类维度的优惠点数（key=ProductCategoryId 字符串，value=DiscountPoints）。
    /// 用于工作流条件字段：CategoryDiscountPoints.{ProductCategoryIdGuid}
    /// </summary>
    public Dictionary<string, decimal> CategoryDiscountPoints { get; set; } = new(StringComparer.OrdinalIgnoreCase);
    public decimal ContractAmount { get; set; }
    public bool ContractNotCompanyTemplate { get; set; }
    /// <summary>是否无 logo（与订单实体一致，供条件分支字段 IsNoLogo 求值）</summary>
    public bool IsNoLogo { get; set; }
    public bool NeedInvoice { get; set; }
    public bool IsShipped { get; set; }
    public LogisticsPaymentMethodId LogisticsPaymentMethodId { get; set; }
...
/// <summary>
/// 按产品分类的合同优惠（写入 order_band）
/// </summary>
public record OrderCategoryContractItem(
    ProductCategoryId ProductCategoryId,
    string CategoryName,
