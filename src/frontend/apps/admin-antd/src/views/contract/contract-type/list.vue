...
const [Grid, gridApi] = useVbenVxeGrid<ContractTypeOptionApi.ContractTypeOptionItem>({
  formOptions: {
    ...
    proxyConfig: {
      ajax: {
        query: async (
          ...
          const list = await getContractTypeOptionList();
          let items = list;
          if (formValues?.name) {
...
function onActionClick(e: OnActionClickParams<ContractTypeOptionApi.ContractTypeOptionItem>) {
  if (e.code === 'edit') {
    ...
    Modal.confirm({
      ...
      onOk: async () => {
        await deleteContractTypeOption(e.row.id);
...
<template>
  <Page auto-content-height>
    ...
    <Grid :table-title="$t('contract.contractTypeList')">
