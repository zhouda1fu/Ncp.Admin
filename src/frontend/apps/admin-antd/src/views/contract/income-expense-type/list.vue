...
const [Grid, gridApi] = useVbenVxeGrid<IncomeExpenseTypeOptionApi.IncomeExpenseTypeOptionItem>({
  formOptions: {
    ...
    proxyConfig: {
      ajax: {
        query: async (
          ...
          const list = await getIncomeExpenseTypeOptionList();
          let items = list;
          if (formValues?.name) {
...
function onActionClick(e: OnActionClickParams<IncomeExpenseTypeOptionApi.IncomeExpenseTypeOptionItem>) {
  if (e.code === 'edit') {
    ...
    Modal.confirm({
      ...
      onOk: async () => {
        await deleteIncomeExpenseTypeOption(e.row.id);
...
<template>
  <Page auto-content-height>
    ...
    <Grid :table-title="$t('contract.incomeExpenseTypeList')">
