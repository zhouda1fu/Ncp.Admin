...
namespace Ncp.Admin.Web.Application.Commands.Contacts;
    ...
    : ICommandHandler<UpdateContactCommand, bool>
{
    ...
    public async Task<bool> Handle(UpdateContactCommand request, CancellationToken cancellationToken)
    {
        ...
        contact.Update(request.Name, request.Phone, request.Email, request.Company, request.GroupId ?? Contact.UnassignedGroupId);
        return true;
    }
}
