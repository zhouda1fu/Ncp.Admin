...
const [Grid, gridApi] = useVbenVxeGrid<ContactGroupApi.ContactGroupItem>({
  formOptions: {
    ...
    proxyConfig: {
      ajax: {
        query: async (
          ...
          const result = await getContactGroupList(params);
          return {
            items: result.items ?? [],
...
function onActionClick(e: OnActionClickParams<ContactGroupApi.ContactGroupItem>) {
  if (e.code === 'edit') {
...
<template>
  <Page auto-content-height>
    ...
    <Grid :table-title="$t('contact.group.list')">
