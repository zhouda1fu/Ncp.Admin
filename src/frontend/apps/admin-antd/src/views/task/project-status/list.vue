...
import {
  ...
  deleteProjectStatus,
} from '#/api/system/project-status';
...
const [Grid, gridApi] = useVbenVxeGrid<ProjectStatusApi.ProjectStatusItem>({
  formOptions: {
    ...
    proxyConfig: {
      ajax: {
        query: async (
          ...
          const list = await getProjectStatusList();
          let items = list;
          if (formValues?.name) {
...
function onActionClick(
  e: OnActionClickParams<ProjectStatusApi.ProjectStatusItem>,
) {
  if (e.code === 'edit') {
    ...
    Modal.confirm({
      ...
      onOk: async () => {
        await deleteProjectStatus(e.row.id);
